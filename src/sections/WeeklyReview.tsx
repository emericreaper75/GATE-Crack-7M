...
export function WeeklyReview() {
  const { weeklyReviews, addWeeklyReview, pyqLogs, mocks, mastery } = useStore();
            ...
            weeklyReviews.slice().reverse().map(wr => (
              <Card key={wr.id} className="hover:border-accent-primary/50 transition-colors">
                <CardHeader className="bg-bg-elevated border-b border-border py-4">
                  <div className="flex justify-between items-center">
                    <CardTitle className="text-lg flex items-center gap-2"><CalIcon className="w-4 h-4 text-accent-primary" /> {wr.weekRange}</CardTitle>
